import { Shadows, PaletteOptions, alpha } from '@mui/material/styles';
const COLOR: string = '#000000';
const shadowsTheme = ({ shadows }: PaletteOptions): Shadows => {
   const TRANSPARENT_ONE: string = shadows?.transparentOne || alpha(COLOR, 0.2);
   const TRANSPARENT_TWO: string =
      shadows?.transparentTwo || alpha(COLOR, 0.14);
